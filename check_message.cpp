...
bool check_message_login(unsigned char* message, unsigned int messageLength) {
    ...
    if (actualOpcode != OPCODE_LOGIN) return false;
    memcpy(&actualLength, message + SIZE_OPCODE + SIZE_SEQNUMBER, SIZE_LEN);
    if (actualLength !=
        messageLength - (SIZE_OPCODE + SIZE_SEQNUMBER + SIZE_LEN + 1))  // L'uno in più è per il carattere di terminazione della stringa
        return false;
    return true;
}
